...
function playMobileSafariReel(clips, startIndex = 0) {
  ...
  const playIndex = () => { const clip = clips[index], version = ++sourceVersion; caption.innerHTML = reelStyle === 'Off' ? '' : `<span class="caption-title">${reelStyle === 'Bold' ? challenges[clip.challenge].title.toUpperCase() : challenges[clip.challenge].title}</span><span class="caption-time">${formatTimestamp(clip.capturedAt)}</span>`; stopMic(); video.pause(); video.onended = null; if (currentUrl) URL.revokeObjectURL(currentUrl); currentUrl = URL.createObjectURL(clip.blob); video.onloadedmetadata = () => { if (version === sourceVersion) video.currentTime = 0; }; video.src = currentUrl; video.load(); if (soundSettings.micVolume === 'on' && clip.micBlob) { micUrl = URL.createObjectURL(clip.micBlob); micAudio = new Audio(micUrl); micAudio.play().catch(() => {}); reelPlayback.micAudio = micAudio; } video.play().catch(() => { $('#reelPlayerToggle').textContent = '▶'; toast('Tap play to start the reel with sound.'); }); video.onseeked = () => { if (micAudio) micAudio.currentTime = video.currentTime; }; video.onended = () => { if (version !== sourceVersion) return; stopMic(); if (index === clips.length - 1) { player.hidden = true; reelPlayback = null; return; } index += 1; reelPlayback.index = index; playIndex(); }; };
  playIndex();
}
