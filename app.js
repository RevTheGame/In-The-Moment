...
// Chromium-based ChatGPT browser continue through the existing export path.
const useSafariExportWorkaround = (() => {
  const userAgent = navigator.userAgent || '';
  return /Safari/i.test(userAgent) && !/(Android|Chrome|Chromium|CriOS|FxiOS|Edg|OPR|Electron|ChatGPT|Codex)/i.test(userAgent);
})();
const now = new Date();
$('#todayWeekday').textContent = now.toLocaleDateString([], { weekday:'long' });
$('#todayMonthDay').textContent = now.toLocaleDateString([], { month:'long', day:'numeric' });
$('#todayDate').setAttribute('aria-label', now.toLocaleDateString([], { weekday:'long', month:'long', day:'numeric' }));
$('#todayDate').dateTime = now.toLocaleDateString('en-CA');
...
async function saveClip(clip) { try { await clipDatabase.then(database => new Promise((resolve, reject) => { const save = database.transaction('clips', 'readwrite').objectStore('clips').put(clip); save.onsuccess = () => resolve(); save.onerror = () => reject(save.error); })); } catch (error) { sessionClips = [...sessionClips.filter(item => item.challenge !== clip.challenge), clip]; } }
async function deleteAllRecordings() { if (!window.confirm('Delete every saved recording? This cannot be undone.')) return; stopReelPlayback(); sessionClips = []; cachedReelClips = []; completed = []; lastBlob = lastMicBlob = null; lastSoundEvents = []; recordingSoundEvents = []; reelUrls.forEach(URL.revokeObjectURL); reelUrls = []; try { const database = await clipDatabase; await new Promise((resolve, reject) => { const request = database.transaction('clips', 'readwrite').objectStore('clips').clear(); request.onsuccess = () => resolve(); request.onerror = () => reject(request.error); }); database.close(); } catch (_) { /* Session-only clips were already cleared. */ } window.location.hash = 'home'; window.location.reload(); }
...
function renderTheme() {
  const dark = themeMode === 'dark', toggle = $('#themeToggle');
  document.body.classList.toggle('dark-mode', dark);
  document.documentElement.style.colorScheme = dark ? 'dark' : 'light';
  document.querySelector('meta[name="theme-color"]').setAttribute('content', dark ? '#000000' : '#dedbd4');
  toggle.innerHTML = `<span aria-hidden="true">${dark ? '☀︎' : '☾'}</span>`;
  toggle.setAttribute('aria-label', dark ? 'Switch to light mode' : 'Switch to dark mode');
  toggle.setAttribute('aria-pressed', String(dark));
}
function toggleTheme() { themeMode = themeMode === 'dark' ? 'light' : 'dark'; localStorage.setItem('itm-theme', themeMode); renderTheme(); }
...
function createVideoRecorder(mediaStream) {
  ...
  // Its video and microphone tracks therefore stay on the same native clock.
...
function prepareRecordingAudio() {
  ...
  try {
    ...
    // Start the context while this is still the record-button gesture, but do
    // not make camera capture wait for it. It is only used for sound playback.
    void recordingAudioContext.resume().catch(() => {});
  } catch (_) {
...
async function loadSoundBuffer(audioContext, soundPath) {
  ...
  // AudioBuffers are decoded per context. This keeps a capture context closing
  // from invalidating the same sound later in preview or export.
  return audioContext.decodeAudioData((await soundDataCache.get(soundPath)).slice(0));
}
async function playSound(isScheduledSound = false) {
  ...
  const playFallbackSound = () => {
    ...
    sound.onended = unlockSoundButton;
  };
  const audioContext = recordingAudioContext;
  if (!audioContext) return playFallbackSound();
  try {
    await audioContext.resume();
    const buffer = await loadSoundBuffer(audioContext, selectedSound.soundPath);
    if (!startTime || recordingStopping) return;
    const source = audioContext.createBufferSource(), gain = audioContext.createGain();
    source.buffer = buffer; gain.gain.value = volume; source.connect(gain).connect(audioContext.destination); source.start(); rememberSound();
    if (isScheduledSound) setTimeout(unlockSoundButton, 120);
    source.onended = () => { source.disconnect(); gain.disconnect(); unlockSoundButton(); };
  } catch (_) {
    playFallbackSound();
  }
}
...
async function connectCamera() {
  try {
    stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal:cameraFacing }, width: { ideal:1280 }, height: { ideal:720 }, frameRate: { ideal:30 } }, audio: true });
...
async function prepareCamera() {
  show('#record'); $('#recordingTitle').textContent = challenges[current].title.toUpperCase(); $('#recordingGuidance').textContent = challenges[current].prompt; $('#timer').textContent = '00:00'; $('#soundTrigger').disabled = true; $('#soundTrigger span:last-child').textContent = 'Start recording to unlock sound'; $('#recordButton').disabled = false; $('#recordButton').classList.remove('is-recording'); $('#recordLabel').textContent = 'TAP TO RECORD'; await connectCamera();
}
...
async function startRecording() {
  ...
  recordingStopping = false; lastBlob = null; lastMicBlob = null; lastSoundEvents = []; recordingSoundEvents = []; lastDuration = 0;
  $('#recordButton').disabled = true; $('#recordLabel').textContent = 'STARTING…';
  try {
    prepareRecordingAudio();
    const recordingChunks = []; let recordingMimeType = '';
    startTime = Date.now(); recordButtonLockedUntil = startTime + 250; $('#recordButton').disabled = false; $('#recordButton').classList.add('is-recording'); $('#recordLabel').textContent = 'RECORDING — TAP TO FINISH'; $('#soundTrigger').disabled = soundSettings.randomSoundPlay; $('#soundTrigger span:last-child').textContent = soundSettings.randomSoundPlay ? 'Sound incoming…' : 'Play the sound';
    timerId = setInterval(() => $('#timer').textContent = formatted(Math.floor((Date.now() - startTime) / 1000)), 250);
    const activeRecorder = recorder = createVideoRecorder(stream);
    activeRecorder.ondataavailable = event => {
      if (session === recordingSession && event.data.size) { recordingChunks.push(event.data); recordingMimeType ||= event.data.type; }
    };
    activeRecorder.onerror = () => {
      if (session !== recordingSession) return;
      recordingSession += 1; recorder = null; recordingStopping = false; clearInterval(timerId); lastBlob = lastMicBlob = null; lastSoundEvents = [];
      releaseRecordingResources(); $('#recordButton').disabled = false; $('#recordButton').classList.remove('is-recording'); $('#recordLabel').textContent = 'TAP TO RECORD';
      show('#challenge'); toast('This recording could not be saved. Please try again.');
    };
    activeRecorder.onstop = async () => {
      if (recorder === activeRecorder) recorder = null;
      if (session !== recordingSession) return;
      const mimeType = recordingMimeType || activeRecorder.mimeType;
      lastBlob = mimeType ? new Blob(recordingChunks, { type:mimeType }) : new Blob(recordingChunks);
      lastSoundEvents = recordingSoundEvents.map(event => ({ ...event }));
      await finalizeRecording(session);
    };
    // One final blob avoids timeslice timestamp drift in constrained encoders.
    activeRecorder.start();
    if (recordingAudioContext) void loadSoundBuffer(recordingAudioContext, challenges[current].soundPath).catch(() => {});
    if (soundSettings.randomSoundPlay) randomSoundTimer = setTimeout(() => { if (startTime) playSound(true); }, (3 + Math.random() * 4) * 1000);
  } catch (error) {
    if (session !== recordingSession) return;
    recordingSession += 1; clearInterval(timerId); recorder = null; recordingStopping = false; lastBlob = lastMicBlob = null; lastSoundEvents = [];
    releaseRecordingResources(); $('#recordButton').disabled = false; $('#recordButton').classList.remove('is-recording'); $('#recordLabel').textContent = 'TAP TO RECORD';
    show('#challenge'); toast('This browser cannot save camera recordings.');
  }
}
function stopRecording() {
  ...
  lastDuration = Math.max(1, Math.floor((Date.now() - startTime) / 1000)); $('#recordButton').disabled = true; $('#recordLabel').textContent = 'SAVING MOMENT…';
  const activeRecorder = recorder;
  if (activeRecorder?.state === 'recording') {
    try { activeRecorder.stop(); return; } catch (_) { /* Fall through to a recoverable save state. */ }
  }
  if (!activeRecorder) { recordingStopping = false; releaseRecordingResources(); show('#challenge'); toast('No video was captured. Please try again.'); }
}
function releaseRecordingResources() {
  clearTimeout(randomSoundTimer); stream?.getTracks().forEach(track => track.stop()); stream = null;
  const preview = $('#cameraPreview'); preview.pause(); preview.srcObject = null;
  recordingAudioContext?.close().catch(() => {});
  recordingAudioContext = null;
  $('#soundTrigger').disabled = true; startTime = null;
}
async function finalizeRecording(session) {
  releaseRecordingResources();
  if (!lastBlob?.size) {
    recordingStopping = false; $('#recordButton').disabled = false; $('#recordButton').classList.remove('is-recording'); $('#recordLabel').textContent = 'TAP TO RECORD';
    show('#challenge'); toast('No video was captured. Please try again.'); return;
  }
  try {
    await saveClip({ challenge: current, day: today, blob: lastBlob, micBlob: lastMicBlob, soundEvents:lastSoundEvents, audioLayout:lastAudioLayout, duration: lastDuration, capturedAt: new Date().toISOString() });
    if (session !== recordingSession) return;
    cachedReelClips = []; reelUrls.forEach(URL.revokeObjectURL); reelUrls = [];
    await refreshCompleted();
    if (session !== recordingSession) return;
    lastBlob = lastMicBlob = null; lastSoundEvents = []; recordingStopping = false;
    $('#recordButton').disabled = false; $('#recordButton').classList.remove('is-recording'); $('#recordLabel').textContent = 'TAP TO RECORD';
    show('#home'); setActiveNav('#home'); toast('Video added to your reel');
  } catch (_) {
    recordingStopping = false; $('#recordButton').disabled = false; $('#recordButton').classList.remove('is-recording'); $('#recordLabel').textContent = 'TAP TO RECORD';
    show('#challenge'); toast('Could not save this clip. Please try again.');
  }
}
...
function playSequentialReel(clips, startIndex = 0) {
  ...
  const stopSounds = () => {
    soundScheduleVersion += 1;
    scheduledSoundSources.forEach(source => { try { source.stop(); } catch (_) { /* The sound already ended. */ } source.disconnect(); });
    scheduledSoundSources.clear();
  };
  const syncSounds = async shouldPlay => {
    stopSounds();
    const events = activeClip?.soundEvents || [];
    ...
    try {
      ...
      await Promise.all(events.map(async event => {
        ...
        if (!ownsPlayback() || scheduleVersion !== soundScheduleVersion || video.paused) return;
        const elapsed = (video.currentTime || 0) - (event.offsetMs || 0) / 1000;
        if (elapsed >= buffer.duration) return;
        const source = reelSoundContext.createBufferSource(), gain = reelSoundContext.createGain();
        source.buffer = buffer; gain.gain.value = event.volume ?? .7; source.connect(gain).connect(reelSoundContext.destination);
        scheduledSoundSources.add(source);
        source.onended = () => { scheduledSoundSources.delete(source); source.disconnect(); gain.disconnect(); };
        source.start(reelSoundContext.currentTime + Math.max(0, -elapsed), Math.max(0, elapsed));
      }));
    } catch (_) { /* Video playback remains available if a sound asset cannot load. */ }
  };
  const startMic = () => {
  ...
  const syncAudio = (shouldPlay = false) => {
    if (activeClip?.audioLayout === 'native') {
      video.muted = soundSettings.micVolume !== 'on';
      video.defaultMuted = video.muted;
      stopMic();
    } else {
      video.muted = false; video.defaultMuted = false;
      if (soundSettings.micVolume !== 'on') micAudio?.pause();
      else {
        startMic();
        if (micAudio) {
          ...
          if (shouldPlay) micAudio.play().catch(() => {});
        }
      }
    }
    void syncSounds(shouldPlay);
  };
  const pauseAudio = () => { micAudio?.pause(); stopSounds(); };
  const cleanup = () => {
    if (cleaned) return;
  ...
  reelPlayback = {
    ...
    pauseMic: pauseAudio,
    resumeMic: () => syncAudio(true),
    syncMic: () => syncAudio(!video.paused)
  };
  ...
  const playIndex = () => {
    ...
    const begin = () => {
      ...
      video.style.opacity = '1'; syncAudio(false);
      const videoPlayback = video.play(); syncAudio(true);
      videoPlayback.catch(() => { if (ownsPlayback() && version === sourceVersion) { $('#reelPlayerToggle').textContent = '▶'; pauseAudio(); toast('Tap play to start the reel with sound.'); } });
    };
    video.onloadeddata = begin; video.oncanplay = begin;
    video.onseeked = () => { if (ownsPlayback() && version === sourceVersion) syncAudio(!video.paused); };
    video.onended = () => {
    ...
    video.onerror = () => { if (ownsPlayback() && version === sourceVersion) { $('#reelPlayerToggle').textContent = '▶'; pauseAudio(); toast('This clip could not be played.'); } };
    video.src = currentUrl; video.load();
  };
  playIndex();
}
async function playReel(startIndex = 0) {
  ...
  return playSequentialReel(clips, startIndex);
}
...
const activateFlipCamera = event => {
  ...
  const now = performance.now();
  if (now - lastFlipActivation < 350) return;
  lastFlipActivation = now;
  flipCamera();
};
...
async function exportVerticalReel() {
  // Use one sequential, conservative export path everywhere. It avoids multiple
  // decoders competing for memory while still preserving the transition frame.
  const ExportAudioContext = useSafariExportWorkaround ? (window.AudioContext || window.webkitAudioContext) : window.AudioContext;
  if (!ExportAudioContext) return toast('This browser cannot create a video export.');
  ...
  const canvasStream = canvas.captureStream(30), canvasTrack = canvasStream.getVideoTracks()[0];
  ...
  let safariKeepAliveSource, safariKeepAliveGain;
  if (useSafariExportWorkaround) {
    try {
      // Safari can discard a MediaStreamDestination that is silent when the
      // recorder begins. Keep an inaudible sample flowing until it finalizes.
    ...
    } catch (_) { /* The normal stream still exports if this Safari aid is unavailable. */ }
  }
  const releaseSafariKeepAlive = () => {
    try { safariKeepAliveSource?.stop(); } catch (_) { /* The source may already be stopped. */ }
    safariKeepAliveSource?.disconnect(); safariKeepAliveGain?.disconnect();
  };
  ...
  const finishRecorder = async () => {
    ...
    const timeout = useSafariExportWorkaround ? 8000 : 3500;
    return Promise.race([finished, new Promise(resolve => setTimeout(() => resolve(new Blob(parts, { type: exportRecorder.mimeType })), timeout))]);
  };
  ...
  const prepareExportItem = async clip => {
    ...
    await waitForExportVideo(video); return { clip, video, url, gain, mic, micUrl, micSource, micGain, soundBuffers, soundSources:[] };
  };
  const startExportSounds = item => {
    if (useSafariExportWorkaround) {
      // Safari may begin decoding video after play() resolves. Hold each sound
      // until the matching video frame actually reaches its saved timestamp.
      item.pendingSoundBuffers = [...item.soundBuffers];
      return;
    }
  ...
  const syncSafariExportSounds = item => {
    if (!useSafariExportWorkaround || !item.pendingSoundBuffers?.length) return;
    const videoTime = item.video.currentTime || 0;
    item.pendingSoundBuffers = item.pendingSoundBuffers.filter(({ event, buffer }) => {
      const eventTime = Math.max(0, event.offsetMs || 0) / 1000;
      if (videoTime + .01 < eventTime) return true;
  ...
  for (let index = 0; index < clips.length && !exportJob.cancelled; index += 1) {
    ...
    await new Promise(resolve => { const render = () => {
      ...
      const transitionProgress = transitionFrame ? Math.min(1, (performance.now() - transitionStartedAt) / transitionFadeDuration) : 1;
      if (transitionFrame && transitionProgress < 1) { context.drawImage(transitionFrame, 0, 0); drawExportFrame(context, canvas, item.video, item.clip, transitionProgress, false); } else { transitionFrame = null; drawExportFrame(context, canvas, item.video, item.clip, 1, true); }
      syncSafariExportSounds(item);
      if (useSafariExportWorkaround) canvasTrack?.requestFrame?.();
      const clipFinished = item.video.ended || (Number.isFinite(item.video.duration) && item.video.currentTime >= item.video.duration - .04) || performance.now() - playbackStartedAt >= expectedPlaybackMs;
      if (exportJob.cancelled || clipFinished) { cancelAnimationFrame(frame); resolve(); } else frame = requestAnimationFrame(render);
    }; render(); });
    if (index < clips.length - 1) { transitionFrame = document.createElement('canvas'); transitionFrame.width = canvas.width; transitionFrame.height = canvas.height; transitionFrame.getContext('2d').drawImage(canvas, 0, 0); }
    item.soundSources.forEach(source => { try { source.stop(); } catch (_) { /* The sound already ended. */ } source.disconnect(); }); item.mic?.pause(); item.micSource?.disconnect(); item.micGain?.disconnect(); if (item.micUrl) URL.revokeObjectURL(item.micUrl); URL.revokeObjectURL(item.url);
  }
  if (stagedExport) { stagedExport.gain.disconnect(); stagedExport.video.pause(); stagedExport.video.remove(); }
  if (exportJob.cancelled) { await finishRecorder(); releaseSafariKeepAlive(); await exportAudioContext.close(); exportJob = null; return; }
  setExportProgress(96); $('#exportStatus').textContent = 'Finishing your reel…';
  const reel = await finishRecorder(); releaseSafariKeepAlive(); await exportAudioContext.close();
