...
function playMobileSafariReel(clips, startIndex = 0) {
  ...
  const playIndex = () => { const clip = clips[index], version = ++sourceVersion; caption.innerHTML = reelStyle === 'Off' ? '' : `<span class="caption-title">${reelStyle === 'Bold' ? challenges[clip.challenge].title.toUpperCase() : challenges[clip.challenge].title}</span><span class="caption-time">${formatTimestamp(clip.capturedAt)}</span>`; stopMic(); video.pause(); video.onended = null; video.style.opacity = '0'; video.playbackRate = video.defaultPlaybackRate = 1; if (currentUrl) URL.revokeObjectURL(currentUrl); currentUrl = URL.createObjectURL(clip.blob); const begin = () => { if (version !== sourceVersion) return; video.currentTime = 0; video.playbackRate = video.defaultPlaybackRate = 1; video.style.opacity = '1'; if (soundSettings.micVolume === 'on' && clip.micBlob) { micUrl = URL.createObjectURL(clip.micBlob); micAudio = new Audio(micUrl); micAudio.play().catch(() => {}); reelPlayback.micAudio = micAudio; } video.play().catch(() => { $('#reelPlayerToggle').textContent = '▶'; toast('Tap play to start the reel with sound.'); }); }; video.onloadeddata = begin; video.src = currentUrl; video.load(); video.onseeked = () => { if (micAudio) micAudio.currentTime = video.currentTime; }; video.onended = () => { if (version !== sourceVersion) return; stopMic(); if (index === clips.length - 1) { player.hidden = true; reelPlayback = null; return; } index += 1; reelPlayback.index = index; playIndex(); }; };
  playIndex();
}
...
async function exportVerticalReel() {
  ...
  for (let index = 0; index < clips.length && !exportJob.cancelled; index += 1) {
    ...
    item.gain.gain.value = 1; item.video.playbackRate = item.video.defaultPlaybackRate = 1; if (item.micGain) item.micGain.gain.value = 1; if (!item.video.currentTime) { await startExportVideo(item.video); if (item.mic) await startExportVideo(item.mic); }
